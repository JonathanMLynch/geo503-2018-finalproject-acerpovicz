...
## NOAA buoy stations information and map
```{r}
...
storm_shp_read() %>%
st_as_sf()
hurricanes1995 <- filter(hurricanes, year == 1995) %>%
...
st_john_hurricane_plot <- ggplot(st_john_data) %>% geom_line(aes(y= wave_height, x= time))
